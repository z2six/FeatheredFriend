...
public class ScrollSealingScreen extends AbstractContainerScreen<ScrollSealingMenu> {
    ...
    private static final ResourceLocation SCROLL_OPENING_TEXTURE =
            new ResourceLocation(Constants.MOD_ID, "textures/gui/scrollscreen/scroll_opening.png");
    ...
    private static final ResourceLocation ALAGARD_FONT_ID =
            new ResourceLocation(Constants.MOD_ID, "alagard");
    ...
    protected void init() {
        ...
        try {
            getScrollMenu().setClientSlotsVisible(false);
        ...
        this.dateWidget = new MultiLineScrollTextWidget(
                ...
                DATE_MAX_CHARS,
                1,
                Component.translatable("screen.featheredfriend.scroll_sealing.field.date"),
                getScrollFontId(),
                false
        );
        ...
        this.recipientField = new MultiLineScrollTextWidget(
                ...
                RECIPIENT_MAX_CHARS,
                1,
                Component.translatable("screen.featheredfriend.scroll_sealing.field.recipient"),
                getScrollFontId(),
                false
        );
        ...
        this.messageWidget = new MultiLineScrollTextWidget(
                ...
                MESSAGE_MAX_CHARS,
                MESSAGE_MAX_LINES,
                Component.translatable("screen.featheredfriend.scroll_sealing.field.message"),
                getScrollFontId(),
                true
        );
        ...
        this.signatureWidget = new MultiLineScrollTextWidget(
                ...
                SIGNATURE_MAX_CHARS,
                2,
                Component.translatable("screen.featheredfriend.scroll_sealing.field.signature"),
                getScrollFontId(),
                false
        );
        ...
        this.recipientOverlay = new RecipientOverlay(
                ...
                new RecipientOverlay.SelectionCallback() {
                    ...
                    public void onPlayerSelected(UUID uuid, String name) {
                        ...
                        selectedRecipientName = name == null ? "" : name.trim();
                        recipientField.setText(buildRecipientPrefillText(selectedRecipientName));
...
// impression behaviour.
    private SigilPattern buildPatternFromStamp(@NotNull ItemStack stampStack) {
        try {
            ...
            CompoundTag root = StackCustomDataUtil.getCopy(stampStack);
            if (root.isEmpty() || !root.contains("SealStamp")) {
...
// independent radius knob: ZOOM_SIGIL_RADIUS_PIXELS.
    ...
    private String computeCurrentDateString() {
        try {
            ...
            if (mc == null || mc.level == null) {
                ...
                return Component.translatable("screen.featheredfriend.scroll_sealing.date.unknown").getString();
            }
    ...
    private String buildFallbackDateString(long dayTime) {
        ...
        return Component.translatable("screen.featheredfriend.scroll_sealing.date.day", dayNumber).getString();
    }

    private static String buildRecipientPrefillText(@NotNull String name) {
        return Component.translatable("screen.featheredfriend.scroll_sealing.recipient.prefilled", name).getString();
    }

    private static String resolveRecipientNameFromText(String recipientTextRaw) {
        if (recipientTextRaw == null) {
            return "";
        }

        String text = recipientTextRaw.trim();
        if (text.isEmpty()) {
            return "";
        }

        String localizedPrefix = Component.translatable(
                "screen.featheredfriend.scroll_sealing.recipient.prefilled_prefix"
        ).getString();
        if (localizedPrefix != null && !localizedPrefix.isBlank()) {
            String prefix = localizedPrefix.trim();
            String lowerText = text.toLowerCase(Locale.ROOT);
            String lowerPrefix = prefix.toLowerCase(Locale.ROOT);
            if (lowerText.startsWith(lowerPrefix)) {
                String candidate = text.substring(Math.min(prefix.length(), text.length())).trim();
    ...
    public void renderBackground(@NotNull GuiGraphics guiGraphics) {
        // Intentionally empty: keep the game visible behind our scroll UI.
        // NeoForge's menu blur helper doesn't exist on Forge 1.20.1.
    }
                ...
                if (button == 0) {
                    if (inZoomWaxArea) {
                        ...
                        try {
                            ...
                            } else if (SealStampSlotEntry.isVirtualSlot(this.sealStampSlotIndex)) {
                                actualStamp = this.sealStampStackForRender == null
                                        ? ItemStack.EMPTY
                                        : this.sealStampStackForRender.copy();
                            } else {
                                LOG.warn("[ScrollSealingScreen] Unexpected stamp slot index {} (expected 0..35, 37, or virtual external slot)", this.sealStampSlotIndex);
                            }
                            ...
                            try {
                                CompoundTag root = StackCustomDataUtil.getCopy(actualStamp);
                                if (root.isEmpty() || !root.contains("SealStamp")) {
                            ...
                            String recipientNameResolved = selectedRecipientName == null ? "" : selectedRecipientName.trim();
                            if (recipientNameResolved.isEmpty()) {
                                recipientNameResolved = resolveRecipientNameFromText(recipientTextRaw);
                            }
                            ...
                            Services.PLATFORM.sendWaxSealToServer(
                                    this.sealStampSlotIndex,
    ...
    private void onEnderPearlClicked() {
        try {
            ...
            if (!hasEnderPearlClient()) {
                notifyMissingEnderPearlClient();
                return;
            }
    ...
    private boolean hasEnderPearlClient() {
        try {
            Minecraft mc = Minecraft.getInstance();
            if (mc == null || mc.player == null) {
                return false;
            ...
            LOG.error("[ScrollSealingScreen] hasEnderPearlClient failed", t);
        }
        return false;
    }

    private void notifyMissingEnderPearlClient() {
        try {
            Minecraft mc = Minecraft.getInstance();
            if (mc == null || mc.player == null) {
                return;
            ...
            LOG.error("[ScrollSealingScreen] notifyMissingEnderPearlClient failed", t);
        }
    }
    ...
    // Slot visibility is controlled via ScrollSealingMenu#setClientSlotsVisible(false) (Slot#isActive()).
