...
/**
 * Geometric 3:
 * - Monumental geometric crests with very large silhouettes.
 * - Strict non-overlap to keep the giant symbols readable.
 * - Motifs: cathedral hex, dual-triangle crest, labyrinth square, orbital pentanet.
 */
public final class SealSigilShapeSetGeometric11 implements SealSigilShapeSet {

    private static final Logger LOG = LogUtils.getLogger();

    private static final double SLICE_ANGULAR_MARGIN = Math.toRadians(2.7);
    private static final int DEFAULT_MIN_CANDIDATE_PIXELS = 16;

    private static final double MAJOR_OVERLAP_STRICT = 0.0;
    private static final double MAJOR_OVERLAP_RELAXED = 0.014;
    private static final double SECONDARY_OVERLAP = 0.024;
    private static final double ACCENT_OVERLAP = 0.036;
    ...
    public void applyShapesInSlice(boolean[][] pixels,
        ...
        try {
            ...
            AngleRange laneMain = slotRange(sliceStart, sliceEnd, laneT(0.50, mirrored), 0.52);
            AngleRange laneSideA = slotRange(sliceStart, sliceEnd, laneT(0.20, mirrored), 0.22);
            AngleRange laneSideB = slotRange(sliceStart, sliceEnd, laneT(0.80, mirrored), 0.22);
            ...
            LOG.error("[SealSigilShapeSetGeometric11] applyShapesInSlice failed", t);
        }
    }
    ...
    private boolean carveHexWebSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double outerR = radius * (primary ? 0.34 : 0.26) * scale;
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveHexWebSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveTriadStarSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double triR = radius * (primary ? 0.35 : 0.27) * scale;
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveTriadStarSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveSquareSpiralSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double outerHalf = radius * (primary ? 0.27 : 0.21) * scale;
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveSquareSpiralSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveOrbitalLatticeSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double orbitR = radius * (primary ? 0.33 : 0.24) * scale;
                int ringOuter = Math.max(2, (int) Math.round(orbitR * 0.40));
                int nodeR = Math.max(1, (int) Math.round(radius * (primary ? 0.022 : 0.017) * scale));
                int nodeCount = primary ? 6 : 4;
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveOrbitalLatticeSymbol failed", t);
        }
        return false;
    }
    ...
    private void carveNodePairAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveNodePairAccent failed", t);
        }
    }
    ...
    private void carveRadialTickAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveRadialTickAccent failed", t);
        }
    }
    ...
    private void carveMicroDiamondAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric11] carveMicroDiamondAccent failed", t);
        }
    }
    ...
    private AngleRange slotRange(double sliceStart, double sliceEnd, double centerT, double widthT) {
        ...
        double half = clamp(widthT * 0.5, 0.03, 0.48);
    ...
    private boolean commitCandidate(boolean[][] base,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric11] commitCandidate failed", t);
            return false;
        }
    }
