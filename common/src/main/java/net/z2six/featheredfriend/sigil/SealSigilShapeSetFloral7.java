// MainFile: neoforge/src/main/java/net/z2six/featheredfriend/sigil/SealSigilShapeSetFloral7.java
...
/**
 * neoforge/src/main/java/net/z2six/featheredfriend/sigil/SealSigilShapeSetFloral7.java
 *
 * Shape set 7 - "Floral 2"
 *
 * Visual goals:
 ...
 */
public final class SealSigilShapeSetFloral7 implements SealSigilShapeSet {

    private static final Logger LOG = LogUtils.getLogger();

    private static final int MIN_MOTIF_PASSES = 2;
    private static final int MAX_MOTIF_PASSES = 5;

    /**
     * Radial bands as normalized radius fractions.
     * Slightly increased from the first version to make everything larger.
     */
    private static final double BAND_INNER_FRACTION = 0.30;
    private static final double BAND_MID_FRACTION   = 0.57;
    private static final double BAND_OUTER_FRACTION = 0.93;

    /**
     * Angular margin to avoid hugging slice borders.
     */
    private static final double SLICE_ANGULAR_MARGIN = Math.toRadians(3.4);
    ...
    public void applyShapesInSlice(boolean[][] pixels,
        ...
        try {
            if (pixels == null || pixels.length == 0) {
                LOG.warn("[SealSigilShapeSetFloral7] applyShapesInSlice called with null/empty pixels");
                return;
            }
            if (slices <= 0) {
                LOG.warn("[SealSigilShapeSetFloral7] slices <= 0 ({}), nothing to do", slices);
                return;
            }
            if (sliceIndex < 0 || sliceIndex >= slices) {
                LOG.warn("[SealSigilShapeSetFloral7] sliceIndex {} out of [0, {}), aborting slice carving",
                        sliceIndex, slices);
                return;
            }
            ...
            if (sliceEnd <= sliceStart) {
                LOG.debug("[SealSigilShapeSetFloral7] Degenerate slice after margin: start={} end={}",
                        sliceStart, sliceEnd);
                return;
            }
            ...
            LOG.debug("[SealSigilShapeSetFloral7] Carving floral 2 slice: sliceIndex={} slices={} passes={}",
                    sliceIndex, slices, passes);

            // 1) Foundational: central blossom + ring frame.
            carveCentralRosette(pixels, cx, cy, radius, radiusSq, rng);
            if (rng.nextFloat() < 0.72f) {
                carveSecondaryRosette(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng);
            }
            carveInnerPetalRing(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng);
            carveOuterLeafWreath(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, 0);

            // 2) High-level motif passes: vines, spirals, lotus fans, leaf clusters.
            for (int pass = 0; pass < passes; pass++) {
                try {
                    int recipe = rng.nextInt(5);
                    switch (recipe) {
                        case 0 -> carveCurvingVineArc(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass);
                        case 1 -> carveSpiralVineMotif(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass);
                        case 2 -> carveLeafClusterMotif(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass);
                        case 3 -> carveLotusFanMotif(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass);
                        case 4 -> carveOuterLeafWreath(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass + 1);
                        default -> carveLeafClusterMotif(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng, pass);
                    }
                } catch (Throwable t) {
                    LOG.error("[SealSigilShapeSetFloral7] Motif pass {} failed in slice {}", pass, sliceIndex, t);
                }
            }

            // 3) Final finishing buds / droplets.
            carveBudField(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng);
            if (rng.nextFloat() < 0.86f) {
                carveDewDropField(pixels, cx, cy, radius, radiusSq, sliceStart, sliceEnd, rng);
            }
            ...
            try {
                ...
                LOG.debug("[SealSigilShapeSetFloral7] applyShapesInSlice: sliceIndex={} markedPixels={}",
                        sliceIndex, count);
            } catch (Throwable t) {
                LOG.error("[SealSigilShapeSetFloral7] counting marked pixels failed", t);
            }

        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] applyShapesInSlice failed", t);
        }
    }
    ...
    private void carveCentralRosette(boolean[][] pixels,
                                     ...
                                     @NotNull RandomSource rng) {
        try {
            // Larger baseScale => big central flower with clearer definition.
            double baseScale = BAND_INNER_FRACTION * (1.25 + rng.nextDouble() * 0.55); // ~0.38-0.54 of radius
            int outerR = Math.max(6, (int) Math.round(radius * baseScale));
            int innerR = Math.max(3, (int) Math.round(outerR * 0.45));

            int petals = 8 + rng.nextInt(4); // 8-11 petals
            carvePetalRosette(pixels, cx, cy, innerR, outerR, petals, cx, cy, radiusSq);

            // Central disc and a small ring for a more ornate core.
            int coreR = Math.max(MIN_DETAIL_RADIUS + 2, outerR / 3);
            carveDisc(pixels, cx, cy, coreR, cx, cy, radiusSq);
            carveRing(pixels, cx, cy, Math.max(1, coreR - 2), coreR + 1, cx, cy, radiusSq);
        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] carveCentralRosette failed", t);
        }
    }

    private void carveSecondaryRosette(boolean[][] pixels,
                                       int cx,
                                       int cy,
                                       int radius,
                                       double radiusSq,
        ...
        try {
            ...
            double angle = sliceStart + span * (0.22 + rng.nextDouble() * 0.56);
            double r = radius * (0.38 + rng.nextDouble() * 0.18);

            int flowerX = cx + (int) Math.round(Math.cos(angle) * r);
            int flowerY = cy + (int) Math.round(Math.sin(angle) * r);
            if (!insideCircle(flowerX, flowerY, cx, cy, radiusSq)) {
                return;
            }

            int outerR = Math.max(4, (int) Math.round(radius * (0.14 + rng.nextDouble() * 0.04)));
            int innerR = Math.max(2, (int) Math.round(outerR * 0.45));
            int petals = 6 + rng.nextInt(3);

            carvePetalRosette(pixels, flowerX, flowerY, innerR, outerR, petals, cx, cy, radiusSq);
            carveDisc(pixels, flowerX, flowerY, Math.max(2, outerR / 3), cx, cy, radiusSq);
        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] carveSecondaryRosette failed", t);
        }
    }
    ...
    private void carvePetalRosette(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carvePetalRosette failed", t);
        }
    }
    ...
    private void carveSinglePetal(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveSinglePetal failed", t);
        }
    }
    ...
    private void carveInnerPetalRing(boolean[][] pixels,
        ...
        try {
            ...
            double baseR = radius * BAND_INNER_FRACTION * (1.45 + rng.nextDouble() * 0.30); // ~0.49â€“0.74 of radius
            int petalCount = 2 + rng.nextInt(2); // 2â€“3 per slice for large, readable shapes
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveInnerPetalRing failed", t);
        }
    }
    ...
    private void carveCurvingVineArc(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveCurvingVineArc failed", t);
        }
    }
    ...
    private void carveThickArc(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveThickArc failed", t);
        }
    }
    ...
    private void carveVineLeaves(boolean[][] pixels,
                                 ...
                                 @NotNull RandomSource rng) {
        try {
            int leafCount = 3 + rng.nextInt(3); // 3â€“5
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveVineLeaves failed", t);
        }
    }

    private void carveSpiralVineMotif(boolean[][] pixels,
                                      int cx,
                                      int cy,
                                      int radius,
                                      double radiusSq,
        ...
        try {
            double span = sliceEnd - sliceStart;
            double centerA = sliceStart + span * (0.22 + rng.nextDouble() * 0.56);
            double angleSpan = span * (0.18 + rng.nextDouble() * 0.20);
            double startA = centerA - angleSpan * 0.5;
            double endA = centerA + angleSpan * 0.5;

            double startR = radius * (0.36 + rng.nextDouble() * 0.14);
            double endR = radius * (0.72 + rng.nextDouble() * 0.12);

            int steps = 10 + rng.nextInt(7);
            int prevX = 0;
            int prevY = 0;
            ...
            for (int i = 0; i <= steps; i++) {
                ...
                double angle = startA + (endA - startA) * t + Math.sin(t * Math.PI * 2.0) * span * 0.03;
                double rr = startR + (endR - startR) * ease;

                int x = cx + (int) Math.round(Math.cos(angle) * rr);
                int y = cy + (int) Math.round(Math.sin(angle) * rr);
                if (!insideCircle(x, y, cx, cy, radiusSq)) {
                    continue;
                }

                if (hasPrev) {
                    int thickness = Math.max(1, radius / 20);
                    carveLine(pixels, prevX, prevY, x, y, thickness, cx, cy, radiusSq);
                }
                prevX = x;
                prevY = y;
                hasPrev = true;

                if (i > 1 && i < steps && i % 3 == 0 && rng.nextFloat() < 0.70f) {
                    int budR = Math.max(1, radius / 18);
                    carveDisc(pixels, x, y, budR, cx, cy, radiusSq);
                }
            }

            if (hasPrev) {
                int spiralR = Math.max(2, radius / 16);
                carveRing(pixels, prevX, prevY, Math.max(1, spiralR - 2), spiralR, cx, cy, radiusSq);
            }
        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] carveSpiralVineMotif failed", t);
        }
    }

    private void carveLotusFanMotif(boolean[][] pixels,
                                    int cx,
                                    int cy,
                                    int radius,
                                    double radiusSq,
        ...
        try {
            double span = sliceEnd - sliceStart;
            double baseAngle = sliceStart + span * (0.18 + rng.nextDouble() * 0.64);
            double baseR = radius * (0.46 + rng.nextDouble() * 0.20);

            int baseX = cx + (int) Math.round(Math.cos(baseAngle) * baseR);
            int baseY = cy + (int) Math.round(Math.sin(baseAngle) * baseR);
            if (!insideCircle(baseX, baseY, cx, cy, radiusSq)) {
                return;
            }

            int petals = 3 + rng.nextInt(3); // 3-5 petals
            int innerR = Math.max(2, radius / 11);
            int outerR = innerR + Math.max(4, radius / 6);
            double fanWidth = Math.toRadians(48.0 + rng.nextDouble() * 24.0);

            for (int i = 0; i < petals; i++) {
                double t = (petals == 1) ? 0.5 : i / (double) (petals - 1);
                double ang = baseAngle - fanWidth * 0.5 + fanWidth * t;
                carveSinglePetal(pixels, baseX, baseY, innerR, outerR, ang, cx, cy, radiusSq);
            }

            int coreR = Math.max(2, innerR / 2 + 1);
            carveDisc(pixels, baseX, baseY, coreR, cx, cy, radiusSq);

            int stemLen = Math.max(3, radius / 7);
            int stemX = baseX - (int) Math.round(Math.cos(baseAngle) * stemLen);
            int stemY = baseY - (int) Math.round(Math.sin(baseAngle) * stemLen);
            carveLine(pixels, baseX, baseY, stemX, stemY, Math.max(1, radius / 24), cx, cy, radiusSq);
        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] carveLotusFanMotif failed", t);
        }
    }
    ...
    private void carveOuterLeafWreath(boolean[][] pixels,
        ...
        try {
            ...
            int leafPairs = 2 + rng.nextInt(2); // 2â€“3 leaf pairs
            int innerR = Math.max(3, radius / 10);
            int outerR = innerR + Math.max(3, radius / 7);
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveOuterLeafWreath failed", t);
        }
    }
    ...
    private void carveLeafClusterMotif(boolean[][] pixels,
        ...
        try {
            ...
            int clusterCount = 1 + rng.nextInt(2); // 1â€“2 clusters
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveLeafClusterMotif failed", t);
        }
    }
    ...
    private void carveLeafCluster(boolean[][] pixels,
                                  ...
                                  int cy) {
        try {
            int leaves = 3 + rng.nextInt(2); // 3â€“4 leaves clustered
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveLeafCluster failed", t);
        }
    }
    ...
    private void carveBudField(boolean[][] pixels,
                               ...
                               @NotNull RandomSource rng) {
        try {
            int budCount = 3 + rng.nextInt(3); // 3â€“5, keep it light
            double span = sliceEnd - sliceStart;
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveBudField failed", t);
        }
    }

    private void carveDewDropField(boolean[][] pixels,
                                   int cx,
                                   int cy,
                                   int radius,
                                   double radiusSq,
                                   ...
                                   @NotNull RandomSource rng) {
        try {
            int drops = 2 + rng.nextInt(4); // 2-5 droplets
            double span = sliceEnd - sliceStart;

            for (int i = 0; i < drops; i++) {
                double t = (i + 0.5) / drops;
                double angle = sliceStart + span * t + (rng.nextDouble() - 0.5) * span * 0.08;
                double rNorm = 0.58 + rng.nextDouble() * (BAND_OUTER_FRACTION - 0.58);
                double rr = radius * rNorm;
                ...
                int outer = Math.max(1, radius / 18);
                carveRing(pixels, x, y, Math.max(0, outer - 1), outer + 1, cx, cy, radiusSq);
                if (rng.nextFloat() < 0.45f) {
                    carveDisc(pixels, x, y, Math.max(1, outer - 1), cx, cy, radiusSq);
                }
            }
        } catch (Throwable t) {
            LOG.error("[SealSigilShapeSetFloral7] carveDewDropField failed", t);
        }
    }
    ...
    private void carveDisc(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveDisc failed", t);
        }
    }
    ...
    private void carveRing(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveRing failed", t);
        }
    }
    ...
    private void carveLine(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carveLine failed", t);
        }
    }
    ...
    private void carvePolygon(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetFloral7] carvePolygon failed", t);
        }
    }
}
