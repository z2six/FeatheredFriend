...
/**
 * Geometric 2:
 * - Bold geometric crests with larger silhouettes and stronger center presence.
 * - Non-overlap remains strict so symbols are easy to read.
 * - Motifs: cathedral hex, dual-triangle crest, labyrinth square, orbital pentanet.
 */
public final class SealSigilShapeSetGeometric10 implements SealSigilShapeSet {

    private static final Logger LOG = LogUtils.getLogger();

    private static final double SLICE_ANGULAR_MARGIN = Math.toRadians(2.9);
    private static final int DEFAULT_MIN_CANDIDATE_PIXELS = 16;

    private static final double MAJOR_OVERLAP_STRICT = 0.0;
    private static final double MAJOR_OVERLAP_RELAXED = 0.012;
    private static final double SECONDARY_OVERLAP = 0.020;
    private static final double ACCENT_OVERLAP = 0.034;
    ...
    public void applyShapesInSlice(boolean[][] pixels,
        ...
        try {
            ...
            AngleRange laneMain = slotRange(sliceStart, sliceEnd, laneT(0.50, mirrored), 0.48);
            AngleRange laneSideA = slotRange(sliceStart, sliceEnd, laneT(0.19, mirrored), 0.25);
            AngleRange laneSideB = slotRange(sliceStart, sliceEnd, laneT(0.81, mirrored), 0.25);
            ...
            LOG.error("[SealSigilShapeSetGeometric10] applyShapesInSlice failed", t);
        }
    }
    ...
    private boolean carveHexWebSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double outerR = radius * (primary ? 0.30 : 0.24) * scale;
                double innerR = outerR * 0.50;
                int edgeOuter = Math.max(1, (int) Math.round(radius * (primary ? 0.024 : 0.018) * scale));
                int edgeInner = Math.max(1, edgeOuter - 1);
                int nodeR = Math.max(1, (int) Math.round(radius * (primary ? 0.022 : 0.017) * scale));
                ...
                int core = Math.max(2, (int) Math.round(innerR * 0.34));
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveHexWebSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveTriadStarSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double triR = radius * (primary ? 0.31 : 0.25) * scale;
                double innerHex = triR * 0.46;
                int edge = Math.max(1, (int) Math.round(radius * (primary ? 0.024 : 0.018) * scale));
                ...
                int core = Math.max(1, (int) Math.round(innerHex * 0.38));
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveTriadStarSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveSquareSpiralSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double outerHalf = radius * (primary ? 0.23 : 0.18) * scale;
                double innerHalf = outerHalf * 0.56;
                int edge = Math.max(1, (int) Math.round(radius * (primary ? 0.026 : 0.020) * scale));
                ...
                int nodeR = Math.max(1, (int) Math.round(radius * 0.019 * scale));
                for (int i = 0; i < 4; i++) {
                    stampDisc(candidate, (int) Math.round(outer[i].x), (int) Math.round(outer[i].y), nodeR, cx, cy, radiusSq);
                }

                int core = Math.max(2, (int) Math.round(innerHalf * 0.56));
                stampRing(candidate, (int) Math.round(c.x), (int) Math.round(c.y), Math.max(0, core - 2), core, cx, cy, radiusSq);
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveSquareSpiralSymbol failed", t);
        }
        return false;
    }
    ...
    private boolean carveOrbitalLatticeSymbol(boolean[][] base,
        ...
        try {
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                double orbitR = radius * (primary ? 0.28 : 0.22) * scale;
                int ringOuter = Math.max(2, (int) Math.round(orbitR * 0.40));
                int nodeR = Math.max(1, (int) Math.round(radius * (primary ? 0.022 : 0.017) * scale));
                int nodeCount = primary ? 5 : 4;
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveOrbitalLatticeSymbol failed", t);
        }
        return false;
    }
    ...
    private void carveNodePairAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveNodePairAccent failed", t);
        }
    }
    ...
    private void carveRadialTickAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveRadialTickAccent failed", t);
        }
    }
    ...
    private void carveMicroDiamondAccent(boolean[][] pixels,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric10] carveMicroDiamondAccent failed", t);
        }
    }
    ...
    private AngleRange slotRange(double sliceStart, double sliceEnd, double centerT, double widthT) {
        ...
        double half = clamp(widthT * 0.5, 0.03, 0.46);
    ...
    private boolean commitCandidate(boolean[][] base,
        ...
        try {
            ...
            LOG.error("[SealSigilShapeSetGeometric10] commitCandidate failed", t);
            return false;
        }
    }
