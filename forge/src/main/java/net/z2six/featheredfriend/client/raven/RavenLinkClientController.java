...
public final class RavenLinkClientController {
    ...
    private static final ResourceLocation RAVEN_LINK_EDGE_BLUR_LOCATION =
            new ResourceLocation("featheredfriend", "shaders/post/raven_link_edge_blur.json");
    ...
    private static final ResourceLocation RAVEN_LINK_TETHER1_SOUND_ID =
            new ResourceLocation("featheredfriend", "ravenlink.tether1");
    ...
    public static void registerGameBus() {
        try {
            ...
            MinecraftForge.EVENT_BUS.register(RavenLinkClientController.class);
            registered = true;
        } catch (Throwable t) {
    ...
    public static void onRenderGuiPost(RenderGuiEvent.Post event) {
        try {
            ...
            renderRavenLinkVisionOverlay(event.getGuiGraphics(), mc, event.getPartialTick());
        } catch (Throwable t) {
    ...
    @SubscribeEvent
    public static void onRenderTickPost(TickEvent.RenderTickEvent event) {
        try {
            if (event.phase != TickEvent.Phase.END) {
                return;
            }
    ...
    private static void applyEdgeGaussianBlur(@org.jetbrains.annotations.NotNull Minecraft mc,
        ...
        try {
            ...
            setPostChainUniformSafe(blurChain, "Radius", radius);
            setPostChainUniformSafe(blurChain, "EdgeMix", edgeMix);
            setPostChainUniformSafe(blurChain, "EdgeStart", edgeStart);
            setPostChainUniformSafe(blurChain, "EdgeEnd", 0.98F);
            setPostChainUniformSafe(blurChain, "EdgePower", 1.55F);
    ...
    private static volatile boolean POSTCHAIN_UNIFORMS_LOOKED_UP = false;
    private static volatile boolean POSTCHAIN_UNIFORMS_READY = false;
    private static volatile @org.jetbrains.annotations.Nullable Field POSTCHAIN_PASSES_FIELD = null;
    private static volatile @org.jetbrains.annotations.Nullable Method POSTPASS_GET_EFFECT_METHOD = null;
    private static volatile @org.jetbrains.annotations.Nullable Field POSTPASS_EFFECT_FIELD = null;
    private static volatile @org.jetbrains.annotations.Nullable Method EFFECT_GET_UNIFORM_METHOD = null;
    private static volatile @org.jetbrains.annotations.Nullable Method UNIFORM_SET_FLOAT_METHOD = null;

    private static void setPostChainUniformSafe(@org.jetbrains.annotations.NotNull PostChain chain,
                                               @org.jetbrains.annotations.NotNull String uniformName,
                                               float value) {
        try {
            if (!POSTCHAIN_UNIFORMS_LOOKED_UP) {
                POSTCHAIN_UNIFORMS_LOOKED_UP = true;
                try {
                    Field f = PostChain.class.getDeclaredField("passes");
                    f.setAccessible(true);
                    POSTCHAIN_PASSES_FIELD = f;
                } catch (Throwable ignored) {
                    POSTCHAIN_PASSES_FIELD = null;
                }
                try {
                    POSTPASS_GET_EFFECT_METHOD = Class.forName("net.minecraft.client.renderer.PostPass")
                            .getDeclaredMethod("getEffect");
                    POSTPASS_GET_EFFECT_METHOD.setAccessible(true);
                } catch (Throwable ignored) {
                    POSTPASS_GET_EFFECT_METHOD = null;
                }
                try {
                    Field f = Class.forName("net.minecraft.client.renderer.PostPass").getDeclaredField("effect");
                    f.setAccessible(true);
                    POSTPASS_EFFECT_FIELD = f;
                } catch (Throwable ignored) {
                    POSTPASS_EFFECT_FIELD = null;
                }
                try {
                    EFFECT_GET_UNIFORM_METHOD = Class.forName("net.minecraft.client.renderer.EffectInstance")
                            .getDeclaredMethod("getUniform", String.class);
                    EFFECT_GET_UNIFORM_METHOD.setAccessible(true);
                } catch (Throwable ignored) {
                    EFFECT_GET_UNIFORM_METHOD = null;
                }
                try {
                    UNIFORM_SET_FLOAT_METHOD = Class.forName("com.mojang.blaze3d.shaders.Uniform")
                            .getDeclaredMethod("set", float.class);
                    UNIFORM_SET_FLOAT_METHOD.setAccessible(true);
                } catch (Throwable ignored) {
                    UNIFORM_SET_FLOAT_METHOD = null;
                }

                POSTCHAIN_UNIFORMS_READY = POSTCHAIN_PASSES_FIELD != null
                        && (POSTPASS_GET_EFFECT_METHOD != null || POSTPASS_EFFECT_FIELD != null)
                        && EFFECT_GET_UNIFORM_METHOD != null
                        && UNIFORM_SET_FLOAT_METHOD != null;
            }

            if (!POSTCHAIN_UNIFORMS_READY
                    || POSTCHAIN_PASSES_FIELD == null
                    || EFFECT_GET_UNIFORM_METHOD == null
                    || UNIFORM_SET_FLOAT_METHOD == null) {
                return;
            ...
            Object passesObj = POSTCHAIN_PASSES_FIELD.get(chain);
            ...
            for (Object pass : passes) {
                ...
                Object effect = null;
                try {
                    if (POSTPASS_GET_EFFECT_METHOD != null) {
                        effect = POSTPASS_GET_EFFECT_METHOD.invoke(pass);
                    } else if (POSTPASS_EFFECT_FIELD != null) {
                        effect = POSTPASS_EFFECT_FIELD.get(pass);
                    }
                } catch (Throwable ignored) {
                    effect = null;
                }
                ...
                Object uniform = null;
                try {
                    uniform = EFFECT_GET_UNIFORM_METHOD.invoke(effect, uniformName);
                } catch (Throwable ignored) {
                    uniform = null;
                }
                if (uniform == null) {
                ...
                try {
                    UNIFORM_SET_FLOAT_METHOD.invoke(uniform, value);
                } catch (Throwable ignored) {
                }
            }
        } catch (Throwable ignored) {
